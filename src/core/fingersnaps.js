...
export function getFingerSnapsMultiplier() {
...
export function buyFingerSnaps() {
  if (!Fingersnaps.isAvailableForPurchase || !Fingersnaps.isAffordable) return false;

  if (NormalChallenge(9).isRunning) {
    Fingersnaps.multiplySameCosts();
  }
  Tutorial.turnOffEffect(TUTORIAL_STATE.FINGERSNAPS);
  Currency.antimatter.subtract(Fingersnaps.cost);
  player.totalTickBought++;
  player.records.thisInfinity.lastBuyTime = player.records.thisInfinity.time;
  player.requirementChecks.permanent.singleFingersnaps++;
  if (NormalChallenge(2).isRunning) player.chall2Pow = 0;
  GameUI.update();
  return true;
}

export function buyMaxFingerSnaps() {
  if (!Fingersnaps.isAvailableForPurchase || !Fingersnaps.isAffordable) return;
  let boughtFingersnaps = false;

  Tutorial.turnOffEffect(TUTORIAL_STATE.FINGERSNAPS);
  if (NormalChallenge(9).isRunning) {
    const goal = Player.infinityGoal;
    let cost = Fingersnaps.cost;
    while (Currency.antimatter.gt(cost) && cost.lt(goal)) {
      Fingersnaps.multiplySameCosts();
      Currency.antimatter.subtract(cost);
      player.totalTickBought++;
      boughtFingersnaps = true;
      cost = Fingersnaps.cost;
    }
  } else {
    const purchases = Fingersnaps.costScale.getMaxBought(player.totalTickBought, Currency.antimatter.value, 1);
    if (purchases === null) {
      return;
    }
  ...
  if (boughtFingersnaps) {
    player.records.thisInfinity.lastBuyTime = player.records.thisInfinity.time;
    if (NormalChallenge(2).isRunning) player.chall2Pow = 0;
  }
}

export function resetFingersnaps() {
  player.totalTickBought = 0;
  player.chall9FingersnapsCostBumps = 0;
}
...
export const Fingersnaps = {
  ...
  get multiplier() {
    return getFingerSnapsMultiplier();
  },

  get current() {
    const fingersnaps = Effarig.isRunning
      ? Effarig.fingersnaps
      : this.baseValue.powEffectOf(DilationUpgrade.fingersnapsPower);
    return player.dilation.active || PelleStrikes.dilation.hasStrike ? dilatedValueOf(fingersnaps) : fingersnaps;
  },

  get cost() {
    return this.costScale.calculateCost(player.totalTickBought + player.chall9FingersnapsCostBumps);
  },
  ...
  get costScale() {
    return new ExponentialCostScaling({
      baseCost: 1000,
      baseIncrease: 10,
      costScale: Player.fingerSnapsMultDecrease,
  ...
  get baseValue() {
    return DC.E3.timesEffectsOf(
      ...
      .times(getFingerSnapsMultiplier().pow(this.totalUpgrades));
  },

  get totalUpgrades() {
    let boughtFingersnaps;
    if (Laitela.continuumActive) boughtFingersnaps = this.continuumValue;
    else boughtFingersnaps = player.totalTickBought;
    return boughtFingersnaps + player.totalTickGained;
  },
...
export const FreeFingersnaps = {
  ...
  get softcap() {
    let softcap = FreeFingersnaps.BASE_SOFTCAP;
    if (Enslaved.has(ENSLAVED_UNLOCKS.FREE_FINGERSNAPS_SOFTCAP)) {
      softcap += 100000;
    }
    return softcap;
  },
  ...
  fromShards(shards) {
    ...
    const priceToCap = FreeFingersnaps.softcap * logTickmult;
    ...
    const costFormulaCoefficient = FreeFingersnaps.GROWTH_RATE / FreeFingersnaps.GROWTH_EXP / logTickmult;
    ...
    // The first term (the big one) is the asymptotically more important term (since FreeFingersnaps.GROWTH_EXP > 1),
    // but is small initially. The second term allows us to continue the pre-cap free fingersnaps upgrade scaling
    // of tickmult per upgrade.
    const boughtToCost = bought => costFormulaCoefficient * Math.pow(
      Math.max(bought, 0), FreeFingersnaps.GROWTH_EXP) + bought;
    const derivativeOfBoughtToCost = x => FreeFingersnaps.GROWTH_EXP * costFormulaCoefficient * Math.pow(
      Math.max(x, 0), FreeFingersnaps.GROWTH_EXP - 1) + 1;
    ...
    let approximation = Math.min(
      desiredCost,
      Math.pow(desiredCost / costFormulaCoefficient, 1 / FreeFingersnaps.GROWTH_EXP)
    );
    let counter = 0;
    ...
    return {
      newAmount: purchases + FreeFingersnaps.softcap,
      nextShards: next,
    };
  }
